...
void keyboard (unsigned char key, int x, int y){
	...
	if(key == 'P' && !keys.heldShiftP){//Change Perspective
...
void drawEntity(){
	...
	if(gamePerspective != Default){
		glColor3f(1, 0, 0);
	    glBegin(GL_QUADS);
		    glVertex2f(enemy.x - PLAYERWIDTH/2, enemy.y - PLAYERHEIGHT/2);
		    glVertex2f(enemy.x + PLAYERWIDTH/2, enemy.y - PLAYERHEIGHT/2);
		    glVertex2f(enemy.x + PLAYERWIDTH/2, enemy.y + PLAYERHEIGHT/2);
		    glVertex2f(enemy.x - PLAYERWIDTH/2, enemy.y + PLAYERHEIGHT/2);
	    glEnd();
	...
	float dx = enemy.x - player.x, dy = enemy.y - player.y;
	float angleRelativeToPlayer = atan(dy/dx);
	float angleRelativeToFov = angleRelativeToPlayer - (player.angleX - (80*DEGREE)/2);
	float entityScreenX = WIDTH - WIDTH*angleRelativeToFov/(80*DEGREE);
	float distanceToPlayer = sqrt(pow(dx, 2) + pow(dy, 2));
	
	printf("------------------------\n");
	printf("angleRelativeToPlayer = %f\n", radtodeg(angleRelativeToPlayer));
	printf("angleRelativeToFov = %f\n", radtodeg(angleRelativeToFov));
	printf("entityScreenX = %i\n", (int)entityScreenX);
	printf("sin(angleRelativeToFov) = %f\n", sin(angleRelativeToFov));
	
	glColor3f(1, 0, 0);
	glPointSize(10);
    glBegin(GL_POINTS);
	    glVertex2f(gameOriginX + entityScreenX, (enemy.y*distanceToPlayer/100)*sin(angleRelativeToFov));
    glEnd();
}
